...
function SignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://localhost:3033/api/auth/signup", {
        method: "POST",
  ...
  return (
    <div className="bg-[#011937] h-screen w-screen text-white pt-[2rem]">
      ...
      <div className="flex flex-col justify-center items-center w-[100%] pb-[4rem]">
        ...
        >
          {errors.form && (
            <div className="mb-4 text-red-500 font-bold">{errors.form}</div>
          )}
          <InputBox
            icon="/Icons/name.png"
            type="text"
          ...
          {errors.confirmPassword && (
            <div className="text-red-500 mb-2 w-[90%] text-left">
              {errors.confirmPassword}
            </div>
          )}
          ...
          <button
            type="submit"
            className="hover:bg-gradient-to-tr from-[#33cccc] from-23% to-[#0099ff] to-54% bg-gradient-to-b from-[#0099cc] to-[#003399] text-[1.5rem] rounded-[27px] font-bold w-[90%] h-[3.5rem] flex justify-center items-center gap-1"
